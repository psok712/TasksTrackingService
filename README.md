...
A: Нужно написать генератор иерархических мок-данных, чтоыб получилось все провтестировать. Вероятно дополнительно потребуется метод репозитория указания ParentTaskId (что-о вроде ITaskRepository.SetParentTask )
